...
class Orchestrator:
    ...
    def generate_and_send_configs(self, config, config_name):
        ...
        print(
            ...
            "f-rate:", config["delay_between_failures_us"],
            )
        ...
        for client in self.clients:
            ...
            cd_dir = self.project_directory + 'configs'
            client.run_cmd(
                'cd '+ cd_dir+';'
                'echo \'' + json.dumps(config) + '\' > ' + config_name + ';')
            # print("sending config to", client.hostname, "with base_port", base_port, "and starting_id", starting_id, "location:", cd_dir)
    ...
    def run(self, config):
        ...
        server_thread.start()
        mem_size = int(config["memory_size"])
        sleeptime=int(mem_size/(1024*1024*8*10))
        print("sleeping for ", sleeptime, " seconds based on memory size ", mem_size)
        ...
        print("sleeping for ", sleeptime, " seconds after exp...")
        for i in tqdm(range(sleeptime)):
            time.sleep(1)
        self.kill()
...
def run_trials(config):
    ...
    while len(runs) < trials:
        ...
        if not success:
            print("ERROR: failed to get stats for trial ", len(runs))
        else:

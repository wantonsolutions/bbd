...
def single_run_fault_rate(stat):
    total_faults = 0
    runtime = float(stat['system']['runtime_s'])
    try:
        for client in stat['clients']:
            faults = int(client['stats']['faults_injected'])
            total_faults += faults
    except:
        print("unalbe to find a fault rate")
        exit(0)
    # print("total_faults", total_faults)
    return total_faults / runtime

def calculate_fault_rate_x_axis(stats):
    fault_rates =[]
    std_errs = []
    for stat in stats:
        single_run_fault_rates=[]
        for r in stat:
            single_run_fault_rates.append(single_run_fault_rate(r))
        fault_rates.append(np.mean(single_run_fault_rates))
        std_errs.append(np.std(single_run_fault_rates))
    # print(fault_rates)
    return fault_rates, std_errs
...
def get_x_axis(stats, name):
    ...
    elif name == "fault rate":
        return get_fault_rate_x_axis(stats)
    else:
...
def get_fault_rate_x_axis(stats):
    fault_rate, stderr = calculate_fault_rate_x_axis(stats)
    return fault_rate

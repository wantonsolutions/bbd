...
def run_fault_rate():
    ...
    # workloads = ["ycsb-w"]
    # workloads=["ycsb-a"]
    # delay_between_failures_us=[10000000,1000000,500000,125000,7812]
    # delay_between_failures_us=[500000]
    rows_per_lock = [1,2,4,8,16]
    delay_between_failures_us=[10000000,1000000,500000,250000,125000,62500,31250,15625,7812,3906,1953,976,488,244,122,1]
    # delay_between_failures_us=[10000000,250000,15625,488,1]
    # delay_between_failures_us=[31250,15625,7812,3906,1953,976]
    # delay_between_failures_us=[1000000,500000]

    orchestrator.boot(config.copy())
    for rpl in rows_per_lock:
        runs=[]
        config["buckets_per_lock"]=str(rpl)
        for delay in delay_between_failures_us:
            lconfig=config.copy()
            lconfig["delay_between_failures_us"]=str(delay)
            runs.append(orchestrator.run_trials(lconfig))
        dirname="data/fault-rate-" + str(rpl) + "-rpl"
        dm.save_statistics(runs, dirname=dirname)
...
def locks_per_row():
    ...
    for rpl in rows_per_lock:
        ...
        lconfig["buckets_per_lock"]=str(rpl)
        runs.append(orchestrator.run_trials(lconfig))
    dirname="data/rows_per_lock"
    dm.save_statistics(runs, dirname=dirname)

def plot_locks_per_row():
    fig, ax = plt.subplots(1,1, figsize=(6,3))
    dirname="data/rows_per_lock"
    stats = dm.load_statistics(dirname=dirname)
    stats=stats[0]
    plot_cuckoo.throughput(ax, stats, x_axis="buckets per lock", decoration=False, label="cuckoo")
    ax.legend()
    ax.set_xlabel("rows per lock")
    ax.set_ylabel("MOPS")
    ax.set_title("ycsb-a")
